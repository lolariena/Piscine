...
/*   Updated: 2023/07/14 13:25:52 by aibonade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <stdlib.h>
...
char	*ft_zero(char *nb, char *base_from, char *base_to)
{
	char	*str;
	int		i;
	int		j;
	...
	if (!ft_baseverif(base_from))
		return (NULL);
	i = ft_passspaces(nb, base_from);
	if (i < 0)
		return (NULL);
	j = 0;
	i = ft_sign(nb, i, &j, base_from);
	if (i < 0)
		return (NULL);
	j = 0;
	while (base_from[j] && base_from[j] != nb[i])
		j++;
	if (!base_from[j])
		return (NULL);
	str = NULL;
...
char	*ft_convert_base(char *nbr, char *base_from, char *base_to)
{
	...
	{
		if (nb == 0)
			str = ft_zero(nbr, base_from, base_to);
		else
